...
CalibratorGrayCode::CalibratorGrayCode(unsigned int _screenCols, unsigned int _screenRows) : Calibrator(_screenCols, _screenRows)
{

    // Create encoder/decoder
    encoder = new EncoderGrayCode(screenCols, screenRows, CodecDirBoth);
    decoder = new DecoderGrayCode(screenCols, screenRows, CodecDirBoth);
...
CalibrationData CalibratorGrayCode::calibrate()
{
    ...
    for(unsigned int i=0; i<nFrameSeq; i++){
        ...
        if(decoder->getDir() & CodecDirHorizontal)
            up[i].create(frames[0].size(), CV_32FC1);
        if(decoder->getDir() & CodecDirVertical)
            vp[i].create(frames[0].size(), CV_32FC1);
